...
function deleteAnimal(id) {
    const animalData = readData().animals.filter((animal) => animal.id !== id);
    const data = readData();
...
app.get("/hotels/:id", (req, res) => {
    ...
    const data = readData();
    const hotel = data.hotels?.find((hotel) => hotel.id === id) || null;
    if (hotel) {
        res.json(hotel);
    }
    else {
...
app.get("/restrictions/:id", (req, res) => {
    const id = parseInt(req.params.id);
    const data = readData();
    const restriction = data.hotel_restrictions?.find((r) => r.id === id) || null;
    if (restriction) {
...
app.get("/bookings/:id", (req, res) => {
    const id = parseInt(req.params.id);
    const data = readData();
    const booking = data.bookings?.find((b) => b.id === id) || null;
    if (booking) {
        res.json(booking);
    }
    else {
...
app.post("/bookings", (req, res) => {
    const { id, client_id, animal_id, hotel_id, pricing_id, start_date, end_date, status, total_price, } = req.body;
    const data = readData();
    ...
    const pricing = data.hotel_pricing?.find((p) => p.id === pricing_id);
    if (!pricing) {
        return res
            .status(400)
            .json({ message: "Указанный тариф не существует" });
    }
    const restriction = data.hotel_restrictions?.find((r) => r.id === pricing.restriction_id);
    if (!restriction) {
    ...
    animal_id.forEach((element) => {
        const animal = data.animals?.find((a) => a.id === element);
        if (!animal) {
            return res
                .status(400)
    ...
    const animalTypeBookings = conflictingBookings.filter((booking) => {
        const bookedAnimal = data.animals?.find((a) => a.id === booking.animal_id);
        return (bookedAnimal &&
...
app.delete("/bookings/:id", (req, res) => {
    const id = parseInt(req.params.id);
    const data = readData();
    if (!data.bookings || !data.bookings.some((b) => b.id === id)) {
        return res
            .status(404)
...
app.get("/pricing/:id", (req, res) => {
    const id = parseInt(req.params.id);
    const data = readData();
    const pricing = data.hotel_pricing?.find((price) => price.id === id) || null;
    if (pricing) {
        res.json(pricing);
    }
    else {
...
app.post("/clients", (req, res) => {
    const { id, last_name, first_name, middle_name, phone, email } = req.body;
    const data = readData();
...
app.delete("/clients/:id", (req, res) => {
    const id = parseInt(req.params.id);
    const clientIndex = readData().clients.findIndex((element) => element.id === id);
    if (clientIndex === -1) {
        res.status(404).json({
            message: "Клиента с запрашиваемым id в системе нет",
        });
    }
    const clientsData = readData().clients.filter((client) => client.id !== id);
    const data = readData();
    data.clients = clientsData;
...
app.get("/animal_types", (req, res) => {
    res.json(readData().animal_types);
});
app.post("/animals", (req, res) => {
    const { id, name, animal_type_id, client_id, breed, gender, weight, color, birth_date, sterilized, description, } = req.body;
    const data = readData();
    const animalData = readData().animals;
    if (id) {
...
app.get("/animals/:id/vaccinations", (req, res) => {
    ...
    const animalVaccinations = data.vaccinations?.filter((v) => v.animal_id === animalId) || [];
...
app.post("/vaccinations", (req, res) => {
    ...
    if (id) {
        ...
        const newId = vaccinationsData.length > 0
...
app.get("/animals/:id/treatments", (req, res) => {
    const animalId = parseInt(req.params.id);
    const data = readData();
    // Проверяем существование животного
    const animal = data.animals?.find((a) => a.id === animalId);
    ...
    const animalTreatments = data.treatments?.filter((t) => t.animal_id === animalId) || [];
    res.json(animalTreatments);
});
app.post("/treatments", (req, res) => {
    const { id, animal_id, treatment_date, treatment_type, description } = req.body;
    const data = readData();
    ...
    if (id) {
        ...
        const newId = treatmentsData.length > 0
...
app.get("/medical_records/:id", (req, res) => {
    const id = parseInt(req.params.id);
    const data = readData();
    const medicalRecord = data.medical_records?.find((r) => r.id === id) || null;
    if (medicalRecord) {
...
app.get("/animals/:id/medical_records", (req, res) => {
    const animalId = parseInt(req.params.id);
    const data = readData();
    // Проверяем существование животного
    const animal = data.animals?.find((a) => a.id === animalId);
    ...
    const animalMedicalRecords = data.medical_records?.filter((r) => r.animal_id === animalId) || [];
    res.json(animalMedicalRecords);
});
app.post("/medical_records", (req, res) => {
    const { id, animal_id, system_user_id, date, diagnosis, treatment, prescriptions, notes, } = req.body;
    const data = readData();
    ...
    if (id) {
        ...
        const newId = medicalRecordsData.length > 0
...
app.delete("/medical_records/:id", (req, res) => {
    const id = parseInt(req.params.id);
    const data = readData();
...
app.get("/animals/:id/medical_history", (req, res) => {
    const animalId = parseInt(req.params.id);
    const data = readData();
    // Проверяем существование животного
    const animal = data.animals?.find((a) => a.id === animalId);
    ...
    const vaccinations = data.vaccinations?.filter((v) => v.animal_id === animalId) || [];
    const treatments = data.treatments?.filter((t) => t.animal_id === animalId) || [];
    const medicalRecords = data.medical_records?.filter((r) => r.animal_id === animalId) || [];
...
app.get("/animal_passports", (req, res) => {
    const data = readData();
    res.json(data.animal_passports || []);
});
app.get("/animal_passports/:id", (req, res) => {
    const id = parseInt(req.params.id);
    const data = readData();
...
app.get("/animals/:id/passport", (req, res) => {
    const animalId = parseInt(req.params.id);
    const data = readData();
    // Проверяем существование животного
    const animal = data.animals?.find((a) => a.id === animalId);
    ...
    const animalPassport = data.animal_passports?.find((p) => p.animal_id === animalId) || null;
    if (animalPassport) {
...
app.post("/animal_passports", (req, res) => {
    const { id, animal_id, last_vet_visit, vet_visit_reason, chronic_diseases, past_diseases, vet_contacts, health_features, } = req.body;
    const data = readData();
    ...
    if (id) {
        ...
        const existingPassport = passportsData.find((p) => p.animal_id === animal_id);
        if (existingPassport) {
            return res.status(400).json({
        ...
        const newId = passportsData.length > 0
...
app.delete("/animal_passports/:id", (req, res) => {
    const id = parseInt(req.params.id);
    const data = readData();
    if (!data.animal_passports ||
...
app.get("/pricings_with_periods", (req, res) => {
    const { hotel_pricing, hotel_restrictions, animal_types, animals, clients, bookings, } = JSON.parse(readFileSync(DATA_PATH, "utf8"));
    const result = hotel_pricing.map((pricing) => {
        const restriction = hotel_restrictions.find((r) => r.id === pricing.restriction_id);
        const animalType = animal_types.find((a) => a.id === restriction?.animal_type_id);
        const relatedBookings = bookings
            .filter((b) => b.pricing_id === pricing.id)
            .map((booking) => {
            const animalsInBooking = [];
            booking.animal_id.forEach((animal_id) => {
                animalsInBooking.push(animals.find((a) => a.id === animal_id));
            });
